...
mainPanel(
    tabsetPanel(
       ...
       tabPanel("Factor Plot", plotOutput("plot2"), helpText("Factor variables are plotted through simple plots. We use the same method of creating two dataframes that we used from Numeric Variable then we create two simple transparent plots on top of eachother. Some of the labels of the plots overlap. This is a bug I am trying to fix but for now the simple solution is the only have one checkbox clicked at a time. Some numerical variables have been transformed to categorical variables and have the prefix fac.")),
